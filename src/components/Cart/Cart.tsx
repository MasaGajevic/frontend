...
export default class Cart extends React.Component {
    ...
    private updateCart() {
        ...
        .then((res: ApiResponse) => {
            ...
            this.setStateCount(res.data.cartCarpets.length);
    ...
    private calculateSum(): number {
        ...
        if (this.state.cart === undefined) {
            ...
            for (const item of this.state.cart?.cartCarpets) {
                sum += item.carpet.carpetPrices[item.carpet.carpetPrices.length-1].price * item.quantity;
            }
        }
    ...
    private updateQuantity(event: React.ChangeEvent<HTMLInputElement>) {
        const carpetId = event.target.dataset.carpetId;
        const newQuantity = event.target.value;

        this.sendCartUpdate({
            carpetId: Number(carpetId),
            quantity: Number(newQuantity),
        });
    }

    private removeFromCart(carpetId: number) {
        this.sendCartUpdate({
            carpetId: Number(carpetId),
            quantity: 0,
        });
                ...
                <Modal size="lg" centered show={ this.state.visible } onHide={ () => this.hideCart() }>
                        ...
                        <Table hover size="sm">
                            ...
                            <tbody>
                                { this.state.cart?.cartCarpets.map(item => {
                                    const price = Number(item.carpet.carpetPrices[item.carpet.carpetPrices.length-1].price).toFixed(2);
                                    const total = Number(item.carpet.carpetPrices[item.carpet.carpetPrices.length-1].price * item.quantity).toFixed(2);
                                        ...
                                        <tr>
                                            ...
                                            <td className="text-right">
                                                <Form.Control type="number" step="1" min="1"
                                                              value={ item.quantity }
                                                              data-carpet-id={ item.carpet.carpetId }
                                            ...
                                            <td>
                                                <FontAwesomeIcon
                                                   icon={ faMinusSquare }
                                                   onClick={ () => this.removeFromCart(item.carpet.carpetId) } />
                                            </td>
                                        </tr>
                                    )
                                }, this) }
                    ...
                    <Modal.Footer>
                        <Button variant="primary" onClick={ () => this.makeOrder() }
                                disabled={ this.state.cart?.cartCarpets.length === 0 }>
                            Make an order
                        </Button>
                    </Modal.Footer>
                </Modal>
